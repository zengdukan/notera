...
jest.mock('../../editor/RendererSurface', () => ({
  RendererSurface: ({ document }: { document: unknown }) => <output>{JSON.stringify(document)}</output>,
}));

const userVersion = {
  versionId: 'version', noteId: 'note', kind: 'USER' as const,
  protectionReason: null, versionName: 'Milestone', displayTitle: 'Old', createdAt: 1,
};
const protection = {
  versionId: 'protection', noteId: 'note', kind: 'SYSTEM_PROTECTION' as const,
  protectionReason: 'BEFORE_HISTORY_RESTORE' as const,
  versionName: null, displayTitle: 'Protected', createdAt: 2,
};
  ...
  it('shows user and protected versions with read-only preview and immutable actions', async () => {
    const client = {
      request: jest.fn(async (key: string) => {
        ...
        if (key === 'history.get') return {
          ref: { source: 'VERSION', versionId: 'version' }, noteId: 'note',
          title: 'Old', document: { type: 'doc', version: 1 }, createdAt: 1,
        };
    ...
    const controller = {
      create: jest.fn(), compare: jest.fn(), copy: jest.fn(), restore: jest.fn(),
    } as unknown as HistoryController;

    render(
      <AppProviders locale="en" queryClient={new QueryClient()}>
        <HistoryModal client={client} profileId="profile" noteId="note" controller={controller} />
      </AppProviders>,
    );
