...
describe('trash controller', () => {
  it('switches to target selection when Main requires a restore target', async () => {
    ...
    await expect(controller.restore({ trashEntryId: 'trash' })).resolves.toBe('target-required');
    await expect(controller.restore({ trashEntryId: 'trash', targetFolderId: 'folder' })).resolves.toBe('restored');
    expect(request).toHaveBeenNthCalledWith(1, 'trash.restore', { trashEntryId: 'trash' });
    expect(request).toHaveBeenNthCalledWith(2, 'trash.restore', {
      trashEntryId: 'trash', targetFolderId: 'folder',
    });
  ...
  it('permanently deletes only the requested trash entry', async () => {
    ...
    expect(request).toHaveBeenCalledWith('trash.deletePermanent', { trashEntryId: 'trash' });
  });
});
