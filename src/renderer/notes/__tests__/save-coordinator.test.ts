...
describe('save coordinator', () => {
  ...
  it('debounces for one second, permits one request in flight, and queues newer edits', async () => {
    let state = createDocumentSession({
      noteId: 'note', title: 'Title', document, contentVersion: 1, savedAt: 1, mode: 'edit',
    });
    ...
    const first = deferred<{ noteId: string; contentVersion: number; savedAt: number }>();
    const save = jest.fn(() => first.promise);
    const coordinator = createSaveCoordinator({ getState: () => state, dispatch, save });
    ...
    expect(save).toHaveBeenCalledWith({ noteId: 'note', title: 'First', document });
    ...
    expect(save).toHaveBeenLastCalledWith({ noteId: 'note', title: 'Second', document });
  });
  ...
  it('flushes immediately, preserves failures, and retries the same draft', async () => {
    ...
    expect(save).toHaveBeenCalledTimes(1);
    expect(state).toMatchObject({ saveState: 'failed', draft: { title: 'Draft' } });
