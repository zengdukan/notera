...
`TrashEntry` 支持 Note 和 Folder 子树，记录 Trash Entry ID、Vault ID、对象类型与 ID、原父目录、删除时间和到期时间。

到期时间固定为删除时间加 30 天。边界比较使用 `now >= expiresAt` 视为已到期。时间加法溢出返回领域错误。

删除普通笔记时生成 Note 回收计划。删除普通目录时一次遍历并生成整个子树的回收计划，保留原有父子层级和每个对象的原位置；根目录永远不能进入回收站。

恢复规则：

- 原父目录仍有效时恢复到原位置；
- 原父目录不存在或仍在回收站时，调用方必须明确提供有效的新目标目录；
- 不得静默恢复到根目录；
- 已到期条目不能通过普通恢复流程恢复，返回 `TRASH_ENTRY_EXPIRED`。

永久删除与到期清理只在领域层产生候选删除计划。数据库删除、附件引用复核和事务提交由上层负责。
