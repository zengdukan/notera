...
export class AttachmentRepository implements AttachmentWriter {
  ...
  private validate(value: StoredAttachment): void {
    ...
    if (
      value.attachment.vaultId !== this.vaultId ||
  ...
  insert(value: StoredAttachment): void {
    this.guard();
    this.validate(value);
    if (this.get(value.attachment.id)) violation();
    const a = value.attachment;
    this.connection()
      .prepare(
        `INSERT INTO attachments(id, blob_id, vault_id, file_name,
  ...
  replace(value: StoredAttachment): void {
    this.guard();
    this.validate(value);
    const current = this.get(value.attachment.id);
    if (!current || current.attachment.createdAt !== value.attachment.createdAt)
      violation();
    const a = value.attachment;
    this.connection()
      .prepare(
        `UPDATE attachments SET blob_id=?, file_name=?, mime_type=?,
  ...
  addReference(reference: AttachmentReference): void {
    this.guard();
    const [column, table, id] = this.source(reference);
    if (
      reference.vaultId !== this.vaultId ||
      !this.get(reference.attachmentId) ||
      !this.connection()
    ...
    this.connection()
      .prepare(
        `INSERT OR IGNORE INTO attachment_references(vault_id, attachment_id,
      source_type, ${column}) VALUES (?, ?, ?, ?)`,
      )
      .run(this.vaultId, reference.attachmentId, reference.source, id);
  }

  removeReference(reference: AttachmentReference): void {
    this.guard();
    const [column, , id] = this.source(reference);
    this.connection()
      .prepare(
        `DELETE FROM attachment_references WHERE vault_id=? AND attachment_id=?
      AND source_type=? AND ${column}=?`,
      )
      .run(this.vaultId, reference.attachmentId, reference.source, id);
  }
  ...
  markGcPending(attachment: Attachment): void {
    this.guard();
    const current = this.get(attachment.id);
    if (!current || attachment.localState !== 'GC_PENDING') violation();
    const ref = this.connection()
      .prepare(
        'SELECT 1 FROM attachment_references WHERE vault_id=? AND attachment_id=? LIMIT 1',
      )
      .get(this.vaultId, attachment.id);
    if (ref) violation();
