...
interface VaultApi {
  readonly attachments: {
    get(id: ReturnType<typeof asAttachmentId>): StoredValue | undefined;
  };
  transaction<T>(
    callback: (tx: {
      notes: { insert(note: Note): void };
      attachments: {
        insert(value: StoredValue): void;
        replace(value: StoredValue): void;
        addReference(
          value: ReturnType<typeof createCurrentNoteAttachmentReference>,
        ): void;
        removeReference(
          value: ReturnType<typeof createCurrentNoteAttachmentReference>,
        ): void;
...
function setup(): VaultApi {
  const db = moduleApi().createVaultDatabase({
...
describe('attachment metadata repository', () => {
  it('copies keys and manifests on every write and read and enforces bounds', () => {
    ...
    const manifest = new Uint8Array([1, 2]);
    db.transaction((tx) =>
      tx.attachments.insert({
        attachment: attachment(),
        fileKey: key,
        manifestVersion: 1,
        manifest,
    ...
    expect(first.fileKey).toEqual(new Uint8Array(32).fill(7));
    expect(first.manifest).toEqual(new Uint8Array([1, 2]));
    first.fileKey.fill(0);
    first.manifest.fill(0);
    expect(db.attachments.get(attachment().id)?.manifest).toEqual(
      new Uint8Array([1, 2]),
    );
    [new Uint8Array(31), new Uint8Array(33)].forEach((fileKey) =>
      expectCode(
        () =>
          db.transaction((tx) =>
            tx.attachments.replace({
              attachment: attachment(),
              fileKey,
              manifestVersion: 1,
      ...
      () =>
        db.transaction((tx) =>
          tx.attachments.replace({
            ...
            fileKey: new Uint8Array(32),
            manifestVersion: 1,
            manifest: new Uint8Array(1024 * 1024 + 1),
          }),
        ),
      'STORAGE_OPERATION_FAILED',
    );
  ...
  it('keeps references idempotent and permits GC_PENDING only after the last removal', () => {
    ...
    db.transaction((tx) => {
      ...
      tx.attachments.addReference(reference);
      tx.attachments.addReference(reference);
    });
    expectCode(
      () =>
        db.transaction((tx) =>
          tx.attachments.markGcPending(
            createAttachment({
              ...storedAttachment,
    ...
    db.transaction((tx) => {
      tx.attachments.removeReference(reference);
      tx.attachments.removeReference(reference);
      tx.attachments.markGcPending(
        createAttachment({
          ...storedAttachment,
    ...
    expect(db.attachments.get(storedAttachment.id)?.attachment.localState).toBe(
      'GC_PENDING',
    );
  });
});
