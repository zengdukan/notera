...
function normalizeEntry(value: unknown, persisted = false): CatalogEntry {
  ...
  if (!Number.isSafeInteger(record.sortOrder) || (record.sortOrder as number) < 0) {
    throw new ApplicationError('OPERATION_FAILED');
  }
...
function decode(bytes: Uint8Array): readonly CatalogEntry[] {
  ...
  if (Object.keys(record).length !== 2 || record.version !== 1 || !Array.isArray(record.entries) || record.entries.length > MAX_PROFILES) throw new Error('shape');
  const entries = record.entries.map((entry) => normalizeEntry(entry, true));
  if (new Set(entries.map(({ localProfileId }) => localProfileId)).size !== entries.length) throw new Error('duplicate');
  return sorted(entries);
}

async function tryRead(path: string): Promise<{ bytes: Uint8Array; entries: readonly CatalogEntry[] } | undefined> {
  try {
...
class Catalog implements ProfileCatalog {
  ...
  constructor(entries: readonly CatalogEntry[], bytes: Uint8Array, private readonly paths: ApplicationPaths, private readonly session: () => string, private readonly writer: SnapshotWriter) {
  ...
  has(id: LocalProfileId): boolean { return this.get(id) !== undefined; }
  list(input: PageRequest, currentId?: LocalProfileId): Page<ProfileSummary> { return paginateCatalog(this.entries, input, currentId); }

  private async commit(nextEntries: readonly CatalogEntry[]): Promise<void> {
    const normalized = sorted(nextEntries);
    if (normalized.length > MAX_PROFILES) throw new ApplicationError('OPERATION_FAILED');
    const next = encode(normalized);
    await this.writer(this.paths.catalog, this.paths.catalogBackup, this.bytes, next, this.session());
  ...
  async add(value: CatalogEntry): Promise<void> {
    ...
    if (this.has(entry.localProfileId)) throw new ApplicationError('OPERATION_FAILED');
    await this.commit([...this.entries, entry]);
  }
  async updateCache(value: CatalogEntry): Promise<void> {
    const entry = normalizeEntry(value);
    if (!this.has(entry.localProfileId)) throw new ApplicationError('ENTITY_NOT_FOUND');
    await this.commit(this.entries.map((item) => item.localProfileId === entry.localProfileId ? entry : item));
  }
...
export async function createProfileCatalog(paths: ApplicationPaths, options: CreateProfileCatalogOptions = {}): Promise<{ readonly catalog: ProfileCatalog; readonly report: RecoveryReport }> {
  const session = options.createSessionName ?? (() => randomBytes(16).toString('hex'));
  const writer = options.writeSnapshot ?? replaceFileWithBackup;
  const primary = await tryRead(paths.catalog);
  const backup = primary === undefined ? await tryRead(paths.catalogBackup) : undefined;
  ...
  if (primary === undefined || recovered.changed) {
    await writer(paths.catalog, paths.catalogBackup, loaded.bytes, nextBytes, session());
  }
  return Object.freeze({
